...
type Fund = { scheme_code:number; scheme:string; short_name:string; category:string; asset_class?:string; exit_load?:string; aum?:number|number[]|null; returns?:Returns; cohort:string; confidence:string; eligible_windows:number; positive_alpha_windows:number; eligible_sharpe_windows:number; sharpe_wins:number; portfolio_months?:string[]; rank?:number };
...
type ScreenerSortKey = "scheme"|"category"|"asset_class"|"exit_load"|"aum"|"p2p"|"1d"|"7d"|"15d"|"30d"|"3m"|"6m"|"1y"|"2y"|"3y"|"5y";
...
export default function Home(){
  ...
  return <main>
    ...
    <section className="screener-section" id="screener"><div className="section-intro split-intro"><div><p className="eyebrow">CONSISTENT PERFORMERS</p><h2>The scheme screen behind the stocks.</h2></div><p>These are the actively managed equity growth schemes currently passing the alpha and risk-adjusted consistency screen. Returns below one year are absolute; returns of one year and longer are annualised CAGR.</p></div>
      ...
      <div className="screener-filters"><label className="search-box"><span>⌕</span><input value={screenQuery} onChange={(event)=>setScreenQuery(event.target.value)} placeholder="Search scheme or category"/></label><label><span>Category</span><select value={screenCategory} onChange={(event)=>setScreenCategory(event.target.value)}><option>All categories</option>{categories.map((category)=><option key={category}>{category}</option>)}</select></label><div><strong>{screenerFunds.length}</strong><span>qualified schemes</span></div></div>
      <div className="screener-scroll"><table className="screener-table"><thead><tr>{[["scheme","Schemes"],["category","Category"],["asset_class","Asset Class"],["exit_load","Exit Load"],["aum","A U M"],...(Object.keys(p2pValues).length?[["p2p",`P2P ${Object.values(p2pValues)[0]?.method}`]]:[]),["1d","1 Day"],["7d","7 Day"],["15d","15 Day"],["30d","30 Day"],["3m","3 Month"],["6m","6 Month"],["1y","1 Year"],["2y","2 Year"],["3y","3 Year"],["5y","5 Year"]].map(([key,label])=><th key={key}><button onClick={()=>toggleScreenSort(key as ScreenerSortKey)}>{label}{screenArrow(key as ScreenerSortKey)}</button></th>)}</tr></thead><tbody>{screenerFunds.map((item)=><tr key={item.scheme_code}><td><strong>{item.short_name}</strong><span>{item.cohort}</span></td><td>{item.category}</td><td><span className="asset-pill">{item.asset_class}</span></td><td className="exit-cell">{item.exit_load}</td><td className="aum-cell">{fmtAum(item.aum as number|null)}</td>{Object.keys(p2pValues).length>0&&<td className={p2pValues[item.scheme_code]?.value>=0?"return-positive":"return-negative"}><strong>{fmtReturn(p2pValues[item.scheme_code]?.value)}</strong><span className="nav-dates">{p2pValues[item.scheme_code]?`${p2pValues[item.scheme_code].start_nav_date.slice(5)} → ${p2pValues[item.scheme_code].end_nav_date.slice(5)}`:"—"}</span></td>}{["1d","7d","15d","30d","3m","6m","1y","2y","3y","5y"].map((key)=>{const value=item.returns?.[key]??(key==="30d"?item.returns?.["1m"]:null);return <td key={key} className={value==null?"return-empty":value>=0?"return-positive":"return-negative"}>{fmtReturn(value)}</td>;})}</tr>)}</tbody></table></div>
      <div className="screener-note"><span>↻ Refresh reruns qualification, performance periods, AUM and portfolio disclosures.</span><span>Returns are NAV-based and exclude loads and taxes.</span></div>
    </section>
