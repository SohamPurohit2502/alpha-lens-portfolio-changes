...
type MoveType = Exclude<(typeof moveTypes)[number],"All moves">;
...
export default function Home(){
  ...
  const [selectedMoveTypes,setSelectedMoveTypes]=useState<MoveType[]>([]);
  ...
  const filtered=useMemo(()=>{
    ...
    const rows=siteData.portfolio_rows.filter((row)=>{const name=fundByCode[row.scheme_code]?.short_name??row.scheme;return(fund==="All funds"||name===fund)&&(!selectedMoveTypes.length||selectedMoveTypes.includes(row.change_type as MoveType))&&(!normalized||`${row.security} ${name}`.toLowerCase().includes(normalized));});
  ...
  },[siteData.portfolio_rows,fund,selectedMoveTypes,query,sort,fundByCode]);
  ...
  const toggleMoveType=(type:(typeof moveTypes)[number])=>{setSelectedMoveTypes((current)=>type==="All moves"?[]:current.includes(type)?current.filter((item)=>item!==type):[...current,type]);setVisible(40);};
  ...
  return <main>
    <header className="topbar"><a className="brand" href="#top" aria-label="Alpha Lens home"><img className="brand-mark" src="./alpha-lens-logo.png" alt="Alpha Lens logo"/><span>ALPHA LENS</span></a><nav aria-label="Primary navigation"><a href="#screener">Screener</a><a href="#funds">Funds</a><a href="#signals">Signals</a><a href="#explorer">Explorer</a></nav><button className="refresh-button compact" onClick={refresh} disabled={refreshing}><span className={refreshing?"spinner":"refresh-icon"}>↻</span>{refreshing?"Refreshing":"Refresh data"}</button></header>
    ...
    <section className="pulse" aria-label="Portfolio change summary">{moveTypes.slice(1).map((type)=><button className={`pulse-card ${type.toLowerCase().replaceAll(" ","-")}`} key={type} onClick={()=>{setSelectedMoveTypes([type]);document.getElementById("explorer")?.scrollIntoView({behavior:"smooth"});}}><span>{type}</span><strong>{counts[type]}</strong><small>View moves ↗</small></button>)}</section>
    ...
    <section className="explorer" id="explorer"><div className="section-intro split-intro"><div><p className="eyebrow">MOVE EXPLORER</p><h2>Inspect every material change.</h2></div><p>Every available monthly exposure is shown. New or exited holdings qualify at ≥0.50% weight; existing holdings qualify at an absolute move of ≥0.50 percentage points.</p></div><div className="explorer-actions"><div><strong>{months.length}-month exposure view</strong><span>{months.slice().reverse().map(monthName).join(" → ")}</span></div><button className="download-button" onClick={downloadExcel}><span>↓</span> Download current explorer</button></div>
      ...
      <div className="type-tabs" aria-label="Move types; select one or more">{moveTypes.map((type)=>{const active=type==="All moves"?!selectedMoveTypes.length:selectedMoveTypes.includes(type);return <button className={active?"active":""} aria-pressed={active} key={type} onClick={()=>toggleMoveType(type)}>{type}<span>{type==="All moves"?siteData.portfolio_rows.length:counts[type]}</span></button>;})}</div><div className="result-meta"><strong>{filtered.length}</strong> matching moves <span className="filter-mode">{selectedMoveTypes.length>1?`${selectedMoveTypes.length} move types combined`:"Select multiple move types to combine them"}</span><span className="sort-hint">Click any column heading to sort</span><button onClick={()=>{setFund("All funds");setSelectedMoveTypes([]);setQuery("");}}>Reset filters</button></div>
    ...
    <footer><div className="footer-brand"><img className="brand-mark" src="./alpha-lens-logo.png" alt=""/><strong>ALPHA LENS</strong></div><p>Portfolio comparisons use the latest 3–4 disclosure months available from the source. Weight changes may reflect trading and market-price movement. Validate conclusions with AMC disclosures.</p><p className="disclaimer">Research screen only · Not investment advice</p></footer>
  </main>;
}
